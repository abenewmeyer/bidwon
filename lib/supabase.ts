...
export default function OnboardingPage() {
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      if (user) {
        // 2. Create the missing company_profiles row with default data
        const { error: dbError } = await supabase.from("company_profiles").upsert({
          id: user.id,
          naics_codes: ["541330"],
          keywords: ["structural engineering", "design", "construction"],
        });
        
        if (dbError) throw dbError;
      }
      
      // 3. Redirect to dashboard to run the first sync
